...
Data <- data.frame(Tipo = unlist(strsplit(paste0(Raw$P15, collapse=", "), ", "))) %>%
  mutate(Tipo = str_to_sentence(Tipo)) %>%
  mutate(Tipo = ifelse(Tipo == "", "No atravesaste situaciones de violencia/discriminacion", Tipo)) %>%
  filter(Tipo %nin% c("No atravesaste situaciones de violencia/discriminacion", "No reaccionaste ante estas situaciones")) %>%
  mutate(Tipo = str_replace_all(Tipo, ";", ",")) %>%
  group_by(Tipo) %>%
  summarise(Cantidad = n()) %>%
  ungroup() %>%
  mutate(Tipo = ifelse(Cantidad < 3, "Otras acciones", Tipo)) %>%
  group_by(Tipo) %>%
...
grafico <- ggplot(Data, aes(y=Tipo, x=Porcentaje, fill=Porcentaje)) +
  ...
  theme_light() +
  labs(y="Tipos de acciones realizadas") +
