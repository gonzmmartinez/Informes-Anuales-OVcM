...
Data <- data.frame(Tipo = unlist(strsplit(paste0(Raw$P23, collapse=", "), ", "))) %>%
  ...
  ungroup() %>%
  mutate(Tipo = ifelse(Cantidad < 3, "Otros temas", Tipo)) %>%
  group_by(Tipo) %>%
...
# Colores
Colores <- colorRampPalette(c("lightgray", "#e54c7c"))(3)
...
grafico <- ggplot(Data, aes(y=Tipo, x=Porcentaje, fill=Porcentaje)) +
  ...
  scale_y_discrete(labels = function(z) str_wrap(z, width = 40)) +
