...
Data <- data.frame(Tipo = unlist(strsplit(paste0(Raw$P21, collapse=", "), ", "))) %>%
  mutate(Tipo = str_to_sentence(Tipo)) %>%
  mutate(Tipo = str_replace_all(Tipo, ";", ",")) %>%
  filter(Tipo != "No atravesé situaciones de violencia") %>%
  group_by(Tipo) %>%
  summarise(Cantidad = n()) %>%
  ungroup() %>%
  mutate(Tipo = ifelse(Cantidad < 3, "Otros lugares de la institución educativa", Tipo)) %>%
  group_by(Tipo) %>%
...
# Colores
Colores <- colorRampPalette(c("lightgray", "#1daa6a"))(3)
