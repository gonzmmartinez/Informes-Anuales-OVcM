...
Data <- data.frame(Genero = factor(c("Diputados y diputadas", "Otras áreas"),
                                   levels=c("Diputados y diputadas", "Otras áreas")),
                   Cantidad = c(32,25)) %>%
...
# Definir colores
Colores <- c("Diputados y diputadas" = "#ec6489",
               "Otras áreas" = "#a9a9a9")
...
# Guardar gráfico
ggsave(filename="Diputados_cargo.png", path=paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/PNG/"),
       plot=grafico, dpi=100, width=7, height=5)
ggsave(filename="Diputados_cargo.svg", path=paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/SVG/"),
