...
Data <- data.frame(Genero = factor(c("Senadores y senadoras", "Otras áreas"),
                                   levels=c("Senadores y senadoras", "Otras áreas")),
                   Cantidad = c(22,5)) %>%
...
# Definir colores
Colores <- c("Senadores y senadoras" = "#ec6489",
               "Otras áreas" = "#a9a9a9")
...
grafico <- ggplot(Data, aes(x="", y=Cantidad, fill=Genero)) +
  ...
  geom_richtext(aes(y = ypos, label = Label), color = ifelse(Data$Porcentaje >= 5, "white", "black"), label.color = NA, family="font",
                show.legend=FALSE, fill=NA, nudge_x = ifelse(Data$Porcentaje >=5, 0.05, 0.65)) +
  scale_fill_manual(name=str_wrap("Cargos de las personas que certificaron",25), values=Colores, labels = function(x) str_wrap(x, width = 30)) + 
...
# Guardar gráfico
ggsave(filename="Senadores_cargo.png", path=paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/PNG/"),
       plot=grafico, dpi=100, width=7, height=5)
ggsave(filename="Senadores_cargo.svg", path=paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/SVG/"),
