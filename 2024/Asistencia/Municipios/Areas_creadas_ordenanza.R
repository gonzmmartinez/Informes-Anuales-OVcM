...
Total <- paste0(paste0("<span style='font-size:30pt'>",
                       ...
                       format(sum(Data$Cantidad), big.mark = ".", decimal.mark = ",", format="d", digits=10),
                       "**</span>"))
...
# Guardar gráfico
ggsave(filename="Areas_creadas_ordenanza.png", path=paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/PNG/"),
       plot=grafico, dpi=100, width=9, height=7)
ggsave(filename="Areas_creadas_ordenanza.svg", path=paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/SVG/"),
