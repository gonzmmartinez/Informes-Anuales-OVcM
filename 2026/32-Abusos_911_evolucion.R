...
grafico <- ggplot(Data, aes(x=Orden, y=Cantidad, group = Accion)) +
  ...
  geom_text(aes(label=Cantidad, color=Accion), size=3, family="font_sans", hjust=0.5, show.legend=FALSE, nudge_y=5) +
  annotate(geom="text", y=-20, x=c(6.5, 18.5, 27.5), label=2024:2026,
           size=7, color="black", family="font_sans") +
  annotate(geom="segment", x=c(1,13,18,30), xend=c(1,13,18,30), y=-15, yend=-27.5,
  ...
  scale_x_discrete(labels = c(str_to_title(month(1:12, label = TRUE, abbr = TRUE, locale = "Spanish_Argentina.utf8")),
                              str_to_title(month(1:12, label = TRUE, abbr = TRUE, locale = "Spanish_Argentina.utf8")),
  ...
  theme(text=element_text(family="font_sans"),
        ...
        axis.text.x = element_text(size=12, margin = margin(t=5,r=0,b=5,l=0)),
