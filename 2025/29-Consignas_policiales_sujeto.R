...
grafico1 <- ggplot(Data %>% filter(Sujeto == "Agresor"), aes(x=Año, y=Cantidad, fill=Tipo)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title.x = element_text(size=15, family="font", margin=margin(t=5)),
        axis.title.y = element_text(size=15, family="font"),
        axis.text.x = element_text(size=10, family="font", margin=margin(t=5)),
...
grafico2 <- ggplot(Data %>% filter(Sujeto == "Víctima"), aes(x=Año, y=Cantidad, fill=Tipo)) +
  geom_col() +
  geom_text(aes(label = formatC(Cantidad, big.mark = ".", decimal.mark = ",", format="fg")),
            vjust = -1, size = 4, color="black", family="font") +
  facet_wrap(~Tipo, ncol=1, scales="free_x")  +
  labs(title="Víctimas",
  ...
  theme(legend.position = "none",
        plot.title = element_text(size= 20, family= "font", face="bold", hjust = 0.5, margin = margin(t=0,r=0,b=10,l=0)),
        plot.caption = element_text(size=8, family="font", face="italic", margin=margin(t=10)),
        strip.text.x = element_text(size=12, family="font", face="bold"),
