...
# Modificar datos
Data <- Raw %>%
  filter(Denuncias %nin% c("Sin dato", "Sin datos")) %>%
  mutate(Denuncias = ifelse(Denuncias == "No", "No", "Sí")) %>%
  mutate(Denuncias = factor(Denuncias, levels=c("Sí", "No"))) %>%
  group_by(Denuncias) %>%
  ...
  ungroup %>%
  mutate(Leyenda = ifelse(Porcentaje >= 3, as.character(Denuncias),
                          paste0(Denuncias, " (", formatC(round(Porcentaje,1), big.mark=".", decimal.mark = ","), "%)")))

Sin_dato <- paste0(paste0("<span style='font-size:12.5pt'>",
                          "_Sin dato: **",
                          formatC(nrow(Raw %>% filter(Denuncias %in% c("Sin dato", "Sin datos"))), big.mark = ".", decimal.mark = ","),
                          "**_</span>"))
...
# Gráfico
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.75, fill=Denuncias)) +
  geom_rect() +
  ...
  xlim(c(1.5, 4.5)) +
  theme_void() +
  scale_fill_manual(name = str_wrap("Existencia de denuncias en relación al caso asistido", 15),
                    values = Colores,
                    labels = function(z) str_wrap(Data$Leyenda[match(z, Data$Denuncias)], width = 20)) +
