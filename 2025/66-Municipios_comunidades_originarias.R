...
# Modificar datos
Data <- Raw %>%
  mutate(Originarias = case_when(Originarias == "Sí" ~ "Si",
                                 Originarias == "Si" ~ "Si",
                                 Originarias == "No" ~ "No",
                                 Originarias == "Sin dato" ~ "Sin dato")) %>%
  filter(Originarias != "Sin dato") %>%
  mutate(Originarias = factor(Originarias, levels=c("Si", "No"))) %>%
  group_by(Originarias) %>%
...
Sin_dato <- paste0(paste0("<span style='font-size:12.5pt'>",
                       "_Sin dato: **",
                       formatC(nrow(Raw %>% filter(Originarias == "Sin dato")), big.mark = ".", decimal.mark = ","),
                       "**_</span>"))
...
# Gráfico
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.75, fill=Originarias)) +
  geom_rect() +
  ...
  xlim(c(1.5, 4.5)) +
  theme_void() +
  scale_fill_manual(name = str_wrap("¿Es una persona perteneciente a comunidades originarias?", 15), values = Colores) +
