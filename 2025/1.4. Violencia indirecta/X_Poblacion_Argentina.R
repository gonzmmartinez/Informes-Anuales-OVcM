...
# Leer datos
Raw <- read.csv(file=paste0(dirname(rstudioapi::getActiveDocumentContext()$path), "/Datos/Poblacion_Argentina_evolucion.csv"))
...
Data <- Raw %>%
  ...
  mutate(Grupo = case_when(Provincia == "Salta" ~ "Salta",
                           Provincia == "Total del país" ~ "Total del país",
                           .default = "Resto de provincias")) %>%
  mutate(Grupo = factor(Grupo, levels=c("Salta", "Total del país", "Resto de provincias"))) %>%
...
Colores <- c("Salta" = "#f78154",
             "Total del país" = "#7E5CAD",
             "Resto de provincias" = "#D5E7B5")

# Gráfico
grafico <- ggplot(Data, aes(x=Año, y=TCAM)) +
  geom_line(aes(color=Grupo, group=Provincia, linewidth=Grupo), lineend = "round") +
  scale_x_continuous(breaks = seq(from=1970, to=2025, by=5),
  ...
  coord_cartesian(xlim=c(1970, 2024), ylim=c(-5,35), clip="off") +
