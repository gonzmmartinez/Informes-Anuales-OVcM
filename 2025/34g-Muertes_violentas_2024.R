...
Raw <- data.frame(Caratula = factor(c("Tienen la carátula provisoria de femicidio",
                                      "Femicidio vinculado",
                                      "Se encuentran en etapa de investigación"),
...
Colores <- c("Tienen la carátula provisoria de femicidio" = "#a5559c",
            "Femicidio vinculado" = "#f2904c",
            "Se encuentran en etapa de investigación" = "#1daa6a")
...
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.5, fill=Caratula)) +
  ...
  coord_polar(theta="y", start=-0.75) +
  xlim(c(1.5, 4.5)) +
  theme_void() +
