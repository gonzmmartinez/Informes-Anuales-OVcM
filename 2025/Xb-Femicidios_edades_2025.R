...
grafico <- ggplot(Data, aes(x=Edades, y=Cantidad, fill=Cantidad)) +
  geom_col() +
  geom_text(aes(label=Cantidad), family="font_sans", color="black", vjust=-1, size=8) +
  geom_text(aes(label=paste0(formatC(round(Porcentaje,1), big.mark = ".", decimal.mark = ","), "%")),
            family="font_sans", color="white", vjust=2, size=10, fontface="bold") +
  labs(x="Rango etario", y="Cantidad") +
  scale_alpha_continuous(range=c(0.6,1)) +
  scale_fill_gradient(low="#a782ec", high="#5ec5d4",
