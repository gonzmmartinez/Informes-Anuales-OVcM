...
# Crear datos
Data <- data.frame(Categoria = c("Sí", "No"),
                   Cantidad = c(5, 1)) %>%
...
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.5, fill=Categoria)) +
  ...
  coord_polar(theta="y") +
  labs(title=str_wrap("Si presenció alguna una situación de discriminación o violencia en la escuela por cuestiones de identidad de género, expresión de género u orientación sexual, ¿Se tomó alguna medida significativa para frenar este tipo de situaciones?",
                      width=60),
       subtitle=str_wrap("Docentes y personal administrativo que respondieron afirmativamente a la pregunta anterior, n = 6", width=60)) +
  xlim(c(1.5, 4.5)) +
  theme_void() +
