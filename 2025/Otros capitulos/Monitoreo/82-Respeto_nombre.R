...
# Crear datos
Data <- data.frame(Categoria = c("Sí", "No tuvimos/tenemos estudiantes trans",
                                 "No" , "Solo si cambiaron su DNI"),
                   Cantidad = c(23, 10, 5, 3)) %>%
  ...
  mutate(Categoria = factor(Categoria, levels=c("Sí", "Solo si cambiaron su DNI", "No", "No tuvimos/tenemos estudiantes trans")))
...
Colores <- c("Sí" = "#d3335e",
             "No tuvimos/tenemos estudiantes trans" = "#206170",
             "No" = "#2b42a0",
             "Solo si cambiaron su DNI" = "#852f8c")
...
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.5, fill=Categoria)) +
  ...
  coord_polar(theta="y") +
  labs(title=str_wrap("En los documentos, listados y comunicaciones internas de la institución, ¿se respeta el nombre autopercibido de los y las estudiantes trans?",
                      width=70),
       subtitle=str_wrap("Total de personas que respondieron la encuesta, n = 41", width=70)) +
  xlim(c(1.5, 4.5)) +
  theme_void() +
