...
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.5, fill=Categoria)) +
  ...
  coord_polar(theta="y") +
  labs(title=str_wrap("¿En la institución los y las estudiantes pueden expresar libremente su identidad de género, expresión de género (vestimenta, formas de llevar el cabello) u orientación sexual?",
                      width=60),
       subtitle="Total de personas que respondieron la encuesta, n = 41") +
  xlim(c(1.5, 4.5)) +
  theme_void() +
