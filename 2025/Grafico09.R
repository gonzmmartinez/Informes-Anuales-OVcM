...
# Modificar datos
Data <- Raw %>%
  mutate(Antecedentes = case_when(Antecedentes == "Sí" ~ "Si",
                                  Antecedentes == "Si" ~ "Si",
                                  Antecedentes == "No" ~ "No",
                                  Antecedentes == "Sin dato" ~ "Sin dato")) %>%
  filter(Antecedentes != "Sin dato") %>%
  mutate(Antecedentes = factor(Antecedentes, levels=c("Si", "No"))) %>%
...
Sin_dato <- paste0(paste0("<span style='font-size:12.5pt'>",
                       "_Sin dato: **",
                       formatC(nrow(Raw %>% filter(Antecedentes == "Sin dato")), big.mark = ".", decimal.mark = ","),
                       "**_</span>"))
...
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.75, fill=Antecedentes)) +
  ...
  xlim(c(1.5, 4.5)) +
  theme_void() +
  scale_fill_manual(name = str_wrap("Antecedentes de violencia contra parejas o ex parejas", 15), values = Colores) +
