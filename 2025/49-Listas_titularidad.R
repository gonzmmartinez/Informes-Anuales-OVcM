...
# Agregar datos
Resultados <- Data %>%
  group_by(Titularidad, Género) %>%
  summarise(Cantidad = n()) %>%
  mutate(Porcentaje = (100 * Cantidad)/sum(Cantidad)) %>%
  ungroup %>%
  mutate(Titularidad = factor(Titularidad, levels=c("Suplente", "Titular"))) %>%
  mutate(Género = ifelse(Género == "Varón", "Varones", "Mujeres"))

# Gráfico
grafico <- ggplot(Resultados, aes(x=Porcentaje, y=Titularidad, fill=Género)) +
  ...
  theme(text=element_text(family="font_sans"),
        ...
        axis.text.y = element_text(size=15, family="font_sans", margin=margin(r=5)),
        ...
        legend.title = element_text(family="font_sans", size=12, margin=margin(b=10)),
        legend.text = element_text(size=12),
        legend.box.margin = margin(t=5,b=5,l=0,r=0),
        legend.key.spacing.y = unit(0.5, "cm"),
        plot.margin = margin(t=10, b=10, l=10, r=10),
        plot.background = element_rect(fill = "white", colour = NA),
        strip.text = element_text(size=20, family="font_sans", face="bold"))
...
ggsave(filename = paste0(filename, ".png"),
       ...
       plot=grafico, dpi=100, width=8, height=2.5)
ggsave(filename = paste0(filename, ".pdf"),
       path = paste0(dirname(rstudioapi::getActiveDocumentContext()$path),"/Graficos/PDF/"),
       plot=grafico, dpi=72, width=8, height=2.5)