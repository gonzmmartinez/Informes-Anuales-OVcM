...
grafico <- ggplot(Data, aes(x=Lugar_del_hecho, y=Cantidad, fill=Lugar_del_hecho)) +
  geom_col() +
  geom_text(aes(label=Cantidad), family="font_sans", color="black", nudge_y=0.4, size=20) +
  geom_text(aes(label=paste0(formatC(round(Porcentaje,1), big.mark = ".", decimal.mark = ","), "%")),
            family="font_sans", color="white", nudge_y=-0.4, size=22.5, fontface="bold") +
  geom_text(aes(label=Lugar_del_hecho), family="font_sans", color="white", nudge_y=-0.9, size=15) +
  labs(x="Lugar del hecho", y="Cantidad") +
  scale_alpha_continuous(range=c(0.6,1)) +
  scale_fill_manual(values = Colores, labels = function(x) str_wrap(x, width = 20)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width=20)) +
  scale_y_continuous(breaks=1:3, limits=c(0, max(Data$Cantidad)*1.2), labels = function(z) round(z,0)) +
  theme_void() +
