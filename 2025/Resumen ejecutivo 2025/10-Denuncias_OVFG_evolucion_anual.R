...
grafico <- ggplot(Data, aes(x=Año, y=Cantidad)) +
  ...
  annotate(geom="segment", y=29037, yend=29037, x=8, xend=9.45, linetype=1, color="#2b42a0", linewidth=1) +
  annotate(geom="text", x=8, y=29037+750,
           label = "Estimación realizada\nen 2.024:\n 29.037 denuncias",
           vjust=0, hjust=0, family="font_sans", fontface="italic", color="#2b42a0", size=4) +
  labs(title="",
       x="Año", y="Cantidad") +
  scale_y_continuous(limits=c(0,max(Data$Cantidad+7000)), labels = function(z) formatC(z, big.mark=".", decimal.mark=",", format="d")) +
  theme_light() +
  scale_fill_gradient2(low="#a782ec", high="#852f8c", mid="#852f8c", midpoint=mean(Data$Cantidad)) +
