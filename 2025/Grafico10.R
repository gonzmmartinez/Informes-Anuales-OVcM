...
# Modificar datos
Data <- Raw %>%
  mutate(Denuncias = case_when(Denuncias == "Sí" ~ "Si",
                               Denuncias == "Si" ~ "Si",
                               Denuncias == "No" ~ "No",
                               Denuncias == "Sin dato" ~ "Sin dato")) %>%
  filter(Denuncias != "Sin dato") %>%
  mutate(Denuncias = factor(Denuncias, levels=c("Si", "No"))) %>%
  group_by(Denuncias) %>%
...
Sin_dato <- paste0(paste0("<span style='font-size:12.5pt'>",
                       "_Sin dato: **",
                       formatC(nrow(Raw %>% filter(Denuncias == "Sin dato")), big.mark = ".", decimal.mark = ","),
                       "**_</span>"))
...
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.75, fill=Denuncias)) +
  ...
  xlim(c(1.5, 4.5)) +
  theme_void() +
  scale_fill_manual(name = str_wrap("Existencia de denuncias en relación al caso asistido", 15), values = Colores) +
