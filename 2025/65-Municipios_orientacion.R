...
# Modificar datos
Data <- Raw %>%
  filter(Orientacion != "Sin dato") %>%
  mutate(Orientacion = ifelse(Orientacion %in% c("Bisexual", "Gay", "Otro"), "Otra", Orientacion)) %>%
  mutate(Orientacion = factor(Orientacion, levels=c("Heterosexual", "Otra"))) %>%
...
Sin_dato <- paste0(paste0("<span style='font-size:12.5pt'>",
                       "_Sin dato: **",
                       formatC(nrow(Raw %>% filter(Orientacion == "Sin dato")), big.mark = ".", decimal.mark = ","),
                       "**_</span>"))
...
Colores <- c("Heterosexual" = "#206170",
             "Otra" = "#cbc2ce")

# Gráfico
grafico <- ggplot(Data, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=2.75, fill=Orientacion)) +
  geom_rect() +
  ...
  xlim(c(1.5, 4.5)) +
  theme_void() +
  scale_fill_manual(name = "Orientación", values = Colores) +
