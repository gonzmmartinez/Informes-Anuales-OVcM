...
# Modificar datos
Data <- Raw %>%
  mutate(Modalidad = ifelse(Modalidad == "En el espacio público / acoso callejero", "En el espacio público/ acoso callejero", Modalidad)) %>%
  filter(Modalidad != "Sin dato") %>%
  group_by(Modalidad) %>%
...
# Gr?fico
grafico <- ggplot(Data, aes(x=Porcentaje, y=reorder(Modalidad, -Ord))) +
  geom_col(aes(alpha = Cantidad), fill="#ff621d") +
  ...
  scale_x_continuous(labels = function(z) paste0(round(z,2), "%"), limits=c(0, 105)) +
