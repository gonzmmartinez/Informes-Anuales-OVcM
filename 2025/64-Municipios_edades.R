...
# Modificar datos
Data <- Raw %>%
  mutate(Rango_etario = str_trim(str_remove_all(Rango_etario, "años"))) %>%
  group_by(Rango_etario, Rango_ord) %>%
  summarise(Cantidad = n())

Tabla <- data.frame(
  Rango_etario = c("5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59",
                   "60-64", "65-69", "75-79", "80-84", "85-89", "90-94", "Sin dato"),
  Rango_ord = 1:18)

Data <- Tabla %>%
  left_join(Data, by="Rango_etario") %>%
  select(-Rango_ord.y) %>%
  mutate(Cantidad = ifelse(is.na(Cantidad), 0, Cantidad)) %>%
  rename(Rango_ord = "Rango_ord.x")
